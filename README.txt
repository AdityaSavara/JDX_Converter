...
Project Description:
The JDX Converter is used to read spectra reference data (typically obtained from NIST) into a useable CSV format. With this module, JDX files can either be converted by providing the molecule data individually or by supplying the converter with a CSV containing all of the molecule information and JDX filenames.
...
This module program requires an executable Python development environment such as Anaconda and Spyder. The module does include files which has dependencies on : JCampSG.py
